...
function moveCard(
  board: any,
  { fromPosition, fromColumnId }: any,
  { toPosition, toColumnId }: any
) {
  const sourceColumn = board.columns.find(
    (column: any) => column.id === fromColumnId
  );
  const destinationColumn = board.columns.find(
    (column: any) => column.id === toColumnId
  );
...
function removeColumn(board: any, column: any) {
  return {
    ...board,
    columns: board.columns.filter(({ id }: any) => id !== column.id),
  };
}
